// Bank statement parsing. Deliberately conservative: when a line cannot be read
// with confidence it is skipped and reported, rather than guessed at, because a
// wrong amount silently imported is worse than a row the user re-enters.
...
async function extractText(buffer, password) {
  ...
  try {
    ...
    const error = new Error('That file could not be read as a PDF.');
    error.status = 422;
    throw error;
  }
  ...
  for (let pageNumber = 1; pageNumber <= doc.numPages; pageNumber += 1) {
    ...
    // Group text items into visual rows by their y position, so a table row
    // arrives as one line rather than a stream of disconnected cells.
    const rows = new Map();
...
module.exports = { extractText, detectBank, parseStatement, parseDate, BANKS };
